...
def run_training():
    ...
    # video_files_test = [os.path.join(config.ST_MAPS_PATH, video_path.split('/')[-1]) for video_path in video_files_test["video"].values]
    # video_files_train = [os.path.join(config.ST_MAPS_PATH, video_path.split('/')[-1]) for video_path in video_files_train["video"].values]
    ...
    video_files_train = ["../data/st_maps/data"]
    video_files_test = ["../data/st_maps/data"]
    # --------------------------------------
    # Build Dataloaders
    # --------------------------------------
